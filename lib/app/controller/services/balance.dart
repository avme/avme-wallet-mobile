    ...
    if (data.containsKey("listenValue")) {
      ...
      response.forEach((mapEntry) {
        ...
        if (key == "AVAX") {
          ...
          if (appState.activeContracts.tokens.contains("$key testnet")) appState.activeContracts.token.updateToken("$key testnet", value);
        }
...
void listenValue(ServiceData param) async {
  ...
  do {
    await Future.delayed(Duration(seconds: seconds), () async {
      ...
      print(tokenValue);
      param.sendPort.send({"listenValue": tokenValue});
    });
...
void watchTokenPriceHistory(ServiceData param) async {
  ...
  if (networkTokenRequest.length != 0) {
    ...
    param.sendPort.send({"networkTokenHistory": jsonDecode(networkTokenHistory), 'waitAvaxPort': socket.sendPort});
  } else
  ...
  if (tokensRequest.length > 0) {
    ...
    param.sendPort.send({"tokensHistory": missingDays});
  }
}
