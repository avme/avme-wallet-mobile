...
class TempDays {
  ...
  Future<Database> _initDB(String filePath) async {
    ...
    Database database = await databaseFactory.openDatabase(path, options: OpenDatabaseOptions(version: 1));
    Print.warning("DROPPING IF EXISTS");
    await database.execute('DROP TABLE IF EXISTS $table;');
    await _createDB(database, 2);
  ...
  Future<Map> recoverLatest()
  async {
    final database = await instance.database;
    List<Map> result = await database.rawQuery('SELECT dateepoch, datetime(dateepoch, \'unixepoch\') as converted FROM $table order by dateepoch desc limit 1;');
    return result.first;
  }
}